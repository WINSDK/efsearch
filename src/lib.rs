...
impl<Metadata> PrefixMatch<Metadata> {
    ...
    pub fn reorder(&mut self) {
        self.items.sort_unstable_by(|a, b| cmp(&a.0, &b.0));
    }
    ...
    pub fn find(&self, prefix: &str) -> Match {
        ...
        let mid = match self.items.binary_search_by(|item| cmp(&item.0, prefix)) {
            Ok(idx) => idx,
        ...
        let mut end = mid;
        while end != self.items.len() && self.items[end + 1].0.starts_with(prefix) {
            end += 1;
        }
